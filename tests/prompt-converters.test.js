...
describe('calculateGoogleBudgetTokens', () => {
    ...
    test('Gemini 3.8 Flash uses Gemini 3 thinking levels without minimal', () => {
        ...
        expect(mod.calculateGoogleBudgetTokens(8192, 'medium', 'gemini-3.8-flash')).toBe('medium');
...
describe('convertGooglePrompt', () => {
    ...
    test('keeps a trailing model prefill on older Gemini Flash models', () => {
        const messages = [
            { role: 'user', content: 'Hi' },
            { role: 'assistant', content: 'Prefill' },
        ];
        const result = mod.convertGooglePrompt(messages, 'gemini-3.5-flash', false, names);
        expect(result.contents.map(c => c.role)).toEqual(['user', 'model']);
